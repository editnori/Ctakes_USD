...
[[ -d "$IN_DIR" ]] || { echo "Missing input dir: $IN_DIR" >&2; exit 2; }
if ! find "$IN_DIR" -type f -name '*.txt' | head -n1 | grep -q .; then
  cat >&2 <<EOF
No .txt notes found under $IN_DIR
- Place ~100 de-identified MIMIC notes under: $BASE_DIR/samples/mimic
...
echo "Building 100-note subset in: $SUBSET_DIR"
...
echo "Done. Subset: $SUBSET_DIR  Outputs: $OUT_BASE"
