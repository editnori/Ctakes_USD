...
- **Writers**: `FileTreeXmiWriter`, `tools.reporting.uima.SimpleConceptCsvWriter`, `tools.reporting.uima.CuiCountSummaryWriter`, and for the drug pipeline `tools.reporting.uima.DrugRxNormCsvWriter` (honours overrides from `resources/SemGroups.txt`).
...
- **Drug pipeline extras**: the override descriptor `resources_override/.../DrugMentionAnnotator.xml` to ensure the drug TypeSystem is available.
...
- The drug pipeline depends on `resources_override/.../DrugMentionAnnotator.xml` so the drug TypeSystem is on the classpath.
