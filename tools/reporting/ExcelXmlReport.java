...
public class ExcelXmlReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if ("summary".equalsIgnoreCase(mode)) {
            ...
            List<List<String>> summary = buildSummaryFast(outDir, runInfo, modules, runLog, cuiCounts);
    ...
    // Try to compute summary metrics by reading per-pipeline report.xml (Clinical Concepts sheet)
    private static SubdirMetrics computeMetricsFromReportIfAny(Path subDir) {
        ...
        if (!Files.isRegularFile(report)) {
            ...
            // Heuristic: newest *.xml that is NOT the local dictionary xml and not the parent summary
            try (DirectoryStream<Path> ds = Files.newDirectoryStream(subDir, p -> {
                ...
                if (newest != null) report = newest; else return null;
